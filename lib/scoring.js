...
export function calculateDealScore(x) {
  ...
  const {financed, monthly, financedGovernmentCharges, upfrontGovernmentCharges} = estimateLoan(x);
  ...
  const benchmark = APRS[x.condition]?.[x.credit] ?? APRS.new[x.credit];
  ...
  const monthlyCashRemaining = income-expenses-monthly-insurance-fuel-maintenance;
  const reserveMonths = Math.max(0,savings-down-upfrontGovernmentCharges)/expenses;
  ...
  if (priceDiff >= 30) caps.push({max:59, reason:"Vehicle price is at least 30% above the estimated market value."});
  else if (priceDiff >= 15) caps.push({max:69, reason:"Vehicle price is at least 15% above the estimated market value."});
  if (feePct >= 15) caps.push({max:64, reason:"Dealer fees are at least 15% of the vehicle price."});
  else if (feePct >= 10) caps.push({max:69, reason:"Dealer fees are at least 10% of the vehicle price."});
  if (monthlyCashRemaining < 0) caps.push({max:59, reason:"Listed essential expenses and estimated transportation costs exceed take-home pay."});
  if (affordability <= 5) caps.push({max:69, reason:"The affordability score is 5 out of 25 or lower based on estimated transportation costs and cash reserves."});
  ...
  return {score, verdict, color, base:Math.round(base), categories:{vehicle,finance,affordability,structure},
    caps, monthly, financed, financedGovernmentCharges, interest, benchmark, aprDiff, term, addonPct, negPct};
}
