...
test("a very weak affordability category cannot receive a green verdict", () => {
  const deal = baseDeal({condition:"new", price:50000, market:50000, apr:6,
    down:5000, income:6500, savings:5000, expenses:4000,
    insurance:120, fuel:120, maintenance:100,
    taxes:1000, titleRegistration:250, otherGovernmentFees:150});
  const score = calculateDealScore(deal);
  assert.equal(score.categories.affordability, 2);
  assert.equal(score.base, 74);
  assert.equal(score.score, 69);
  assert.equal(score.color, "yellow");
  assert.ok(score.caps.some(cap => cap.reason.startsWith("The affordability score")));
});

test("the affordability cap starts at 5 of 25 and leaves 6 of 25 uncapped", () => {
  const deal = baseDeal({condition:"new", price:50000, market:50000, apr:6,
    down:5000, income:7200, savings:10000, expenses:4000,
    insurance:120, fuel:120, maintenance:100,
    taxes:1000, titleRegistration:250, otherGovernmentFees:150});
  const atFive = calculateDealScore(deal);
  const atSix = calculateDealScore({...deal, income:5800, savings:20000});
  assert.equal(atFive.categories.affordability, 5);
  assert.equal(atFive.score, 69);
  assert.equal(atSix.categories.affordability, 6);
  assert.equal(atSix.caps.some(cap => cap.reason.startsWith("The affordability score")), false);
  assert.equal(atSix.score, 78);
});
