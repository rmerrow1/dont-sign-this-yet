...
test("compares a valid zero-percent APR payment", () => {
  const zero = {...deal, apr:0};
  const check = checkDealerNumbers(zero, {...details, payment:341.67});
  assert.equal(check.estimatedPayment, 20500 / 60);
  assert.equal(check.comparisons, 2);
  assert.deepEqual(check.notes, []);
});
